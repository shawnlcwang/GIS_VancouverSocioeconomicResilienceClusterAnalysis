...
outputFolder = "D:\\MSc Thesis\\CCAR Database\\2006\\Resilience_2006_Output2"
...
def null_process(path):
    ...
    if name == duplicated_name("Labour_Occupations_Education_DA", 0):
        ...
        null_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["POPULATIO1"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
                cursor.deleteRow()
        del row
        ...
        null_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["TOTAL_PO2"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
                cursor.deleteRow()
        del row
        ...
        null_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["TOTAL_PO5"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
                cursor.deleteRow()
        del row
        ...
        null_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["TOTAL_NU6"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
                cursor.deleteRow()
        del row
        ...
        null_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["TOTAL_NU7"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
                cursor.deleteRow()
        del row
        ...
        null_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["TOTAL_NU9"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
                cursor.deleteRow()
        del row
        ...
        null_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["TOTAL_NU8"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
                cursor.deleteRow()
        del row
...
def fcs_in_workspace(inputFolder):
    ...
    for fc in fclist:
        ...
        if name == duplicated_name("Income_Earnings_DA", 0):
            ...
            keepFields = ["FID", "Shape", "DAUID", "CSDUID", "CCSUID", "CDUID", "ERUID", "PRUID", "CTUID", "CMAUID", "OID_", "GEOGRAPHY", "2001_TO_20", "POPULATIO1", "TFvariable", "Z_score"]
            ...
            keepFields = ["FID", "Shape", "CTUID", "CMAUID", "PRUID", "CT_UID", "OID_", "GEOGRAPHY", "2001_TO_20", "POPULATIO1", "TFvariable", "Z_score"]
            expression0 = "!2001_TO_20! / !POPULATIO1!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
