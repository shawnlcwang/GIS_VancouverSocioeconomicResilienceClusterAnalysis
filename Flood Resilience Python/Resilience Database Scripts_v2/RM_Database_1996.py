...
outputFolder = "D:\\MSc Thesis\\CCAR Database\\1996\\Resilience_1996_Output2"
...
def null_process(path):
    ...
    if name == duplicated_name("Income_EA", 1):
        ...
        null_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["TOTAL_PO3"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
                cursor.deleteRow()
        del row
...
def fcs_in_workspace(inputFolder):
    ...
    for fc in fclist:
        ...
        if name == duplicated_name("Income_EA", 0):
            ...
            keepFields = ["FID", "Shape", "GEOGRAPHY", "TOTAL_LONE", "TOTAL_NUMB", "TFvariable", "Z_score"]
            expression0 = "!TOTAL_LONE! / !TOTAL_NUMB!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
