...
outputFolder = "D:\\MSc Thesis\\CCAR Database\\2011\\Resilience_2011_Output2"
...
def null_process(path):
    ...
    if name == duplicated_name("Income_DA", 1):
        ...
        null_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["KNOWLEDGE_"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
                cursor.deleteRow()
        del row
        ...
        null_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["TOTAL_PO6"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
                cursor.deleteRow()
        del row
        ...
        null_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["TOTAL_NUMB1"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
                cursor.deleteRow()
        del row
        ...
        null_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["TOTAL_POP1"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
                cursor.deleteRow()
        del row
