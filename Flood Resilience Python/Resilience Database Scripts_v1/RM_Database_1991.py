...
def Tvariable_process(path):
    ...
    if name == duplicated_name("Dwelling_households_EA", 0):
        ...
        cursor = arcpy.da.UpdateCursor(path, ["COL6", "Tvariable"])
        for row in cursor:
        ...
        Tvariable_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["COL47", "Tvariable"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
                row[0] = -9999
                row[1] = "Zero_Data"
        ...
        Tvariable_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["COL43", "Tvariable"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
                row[0] = -9999
                row[1] = "Zero_Data"
        ...
        Tvariable_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["COL70", "Tvariable"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
                row[0] = -9999
                row[1] = "Zero_Data"
        ...
        Tvariable_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["COL111", "Tvariable"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
                row[0] = -9999
                row[1] = "Zero_Data"
        ...
        Tvariable_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["COL138", "Tvariable"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
                row[0] = -9999
                row[1] = "Zero_Data"
...
def fcs_in_workspace(inputFolder):
    ...
    for fc in fclist:
        ...
        if name == duplicated_name("Dwelling_households_EA", 0):
            ...
            keepFields = ["FID", "Shape", "EA", "EA_STR", "OID_", "COL6", "COL8", "Tvariable", "TFvariable", "Z_score"]
            ...
            keepFields = ["FID", "Shape", "EA", "EA_STR", "OID_", "COL6", "COL17", "Tvariable", "TFvariable", "Z_score"]
            ...
            keepFields = ["FID", "Shape", "EA", "EA_STR", "OID_", "COL27", "COL34", "Tvariable", "TFvariable", "Z_score"]
            expression0 = "!COL34! / !COL27!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Lone_Parent_Dwellings_1991_DA.shp"
        elif name == duplicated_name("Dwelling_households_B_EA", 2):
            keepFields = ["FID", "Shape", "EA", "EA_STR", "OID_", "COL6", "COL12", "Tvariable", "TFvariable", "Z_score"]
            expression0 = "!COL12! / !COL6!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Dwellings_Major_Repair_Dwellings_1991_DA.shp"
        elif name == duplicated_name("Dwelling_households_B_EA", 3):
            keepFields = ["FID", "Shape", "EA", "EA_STR", "OID_", "COL6", "COL13", "COL14", "Tvariable", "TFvariable", "Z_score"]
            expression0 = "(!COL13! + !COL14!) / !COL6!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Dwellings_1960Construction_Dwellings_1991_DA.shp"
        elif name == duplicated_name("Families_EA", 0):
            keepFields = ["FID", "Shape", "EA", "EA_STR", "OID_", "COL47", "COL51", "Tvariable", "TFvariable", "Z_score"]
            ...
            keepFields = ["FID", "Shape", "EA", "EA_STR", "OID_", "COL7", "COL9", "Tvariable", "TFvariable", "Z_score"]
            expression0 = "!COL9! / !COL7!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Labour_Unemployment_Labour_1991_DA.shp"
        elif name == duplicated_name("Language_B_EA_", 0):
            keepFields = ["FID", "Shape", "EA", "EA_STR", "OID_", "COL6", "COL45", "Tvariable", "TFvariable", "Z_score"]
            expression0 = "!COL45! / !COL6!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Neither_Language_Language_1991_DA.shp"
        elif name == duplicated_name("Reli_ethnic_ori_citzn_immi_B_EA_", 0):
            keepFields = ["FID", "Shape", "EA", "EA_STR", "OID_", "COL6", "COL87", "Tvariable", "TFvariable", "Z_score"]
            expression0 = "!COL87! / !COL6!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Immigration_Religion_1991_DA.shp"
        elif name == duplicated_name("Reli_ethnic_ori_citzn_immi_B_EA_", 1):
            keepFields = ["FID", "Shape", "EA", "EA_STR", "OID_", "COL6", "COL42", "Tvariable", "TFvariable", "Z_score"]
            ...
            keepFields = ["FID", "Shape", "AREA", "PERIMETER", "G91CT0_", "G91CT0_ID", "CA", "CT_NAME", "PROV", "CA_CTNAME", "OID_", "COL43", "COL45", "Tvariable", "TFvariable", "Z_score"]
            ...
            keepFields = ["FID", "Shape", "AREA", "PERIMETER", "G91CT0_", "G91CT0_ID", "CA", "CT_NAME", "PROV", "CA_CTNAME", "OID_", "COL4", "COL117", "Tvariable", "TFvariable", "Z_score"]
            ...
            keepFields = ["FID", "Shape", "AREA", "PERIMETER", "G91CT0_", "G91CT0_ID", "CA", "CT_NAME", "PROV", "CA_CTNAME", "OID_", "COL47", "COL49", "Tvariable", "TFvariable", "Z_score"]
            ...
            keepFields = ["FID", "Shape", "AREA", "PERIMETER", "G91CT0_", "G91CT0_ID", "CA", "CT_NAME", "PROV", "CA_CTNAME", "OID_", "COL70", "COL88", "Tvariable", "TFvariable", "Z_score"]
            ...
            keepFields = ["FID", "Shape", "AREA", "PERIMETER", "G91CT0_", "G91CT0_ID", "CA", "CT_NAME", "PROV", "CA_CTNAME", "OID_", "COL111", "COL115", "Tvariable", "TFvariable", "Z_score"]
            ...
            keepFields = ["FID", "Shape", "AREA", "PERIMETER", "G91CT0_", "G91CT0_ID", "CA", "CT_NAME", "PROV", "CA_CTNAME", "OID_", "COL138", "COL143", "Tvariable", "TFvariable", "Z_score"]
            ...
            keepFields = ["FID", "Shape", "AREA", "PERIMETER", "G91CT0_", "G91CT0_ID", "CA", "CT_NAME", "PROV", "CA_CTNAME", "OID_", "COL47", "COL58", "Tvariable", "TFvariable", "Z_score"]
            ...
            keepFields = ["FID", "Shape", "AREA", "PERIMETER", "G91CT0_", "G91CT0_ID", "CA", "CT_NAME", "PROV", "CA_CTNAME", "OID_", "COL138", "COL145", "COL146", "Tvariable", "TFvariable", "Z_score"]
