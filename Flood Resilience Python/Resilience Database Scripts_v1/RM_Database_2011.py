...
inputFolder = "D:\\MSc Thesis\\CCAR Database\\2011\\Resilience_2011_Input"
outputFolder = "D:\\MSc Thesis\\CCAR Database\\2011\\Resilience_2011_Output"
...
def Tvariable_process(path):
    ...
    if name == duplicated_name("Income_DA", 1):
        ...
        del row
        del cursor
    elif name == duplicated_name("Income_DA", 5):
        Tvariable_identify(path)
        ...
        del row
        del cursor
    elif name == duplicated_name("Income_DA", 6):
        Tvariable_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["TOTAL_NUMB", "Tvariable"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
        ...
        Tvariable_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["KNOWLEDGE_", "Tvariable"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
                row[0] = -9999
                row[1] = "Zero_Data"
        ...
        Tvariable_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["TOTAL_NUMB1", "Tvariable"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
                row[0] = -9999
                row[1] = "Zero_Data"
        ...
        del row
        del cursor
    elif name == duplicated_name("Immigration_CT", 0):
        Tvariable_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["TOTAL_POP1", "Tvariable"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
                row[0] = -9999
                row[1] = "Zero_Data"
        ...
        del row
        del cursor
    elif name == duplicated_name("Immigration_CT", 1):
        Tvariable_identify(path)
...
def fcs_in_workspace(inputFolder):
    ...
    for fc in fclist:
        ...
        if name == duplicated_name("Income_DA", 0):
        ...
        elif name == duplicated_name("Income_DA", 4):
            ...
            keepFields = ["FID", "Shape", "DAUID", "CDUID", "CDNAME", "CDTYPE", "CSDUID", "CSDNAME", "CSDTYPE", "CCSUID", "CCSNAME", "ERUID", "ERNAME", "CMAPUID", "CMAUID", "CMANAME", "CMATYPE", "SACCODE", "SACTYPE", "CTUID", "CTNAME", "PRUID", "PRNAME", "OID_", "GEOGRAPHY", "1960_OR_BE", "TOTAL_NUMB", "Tvariable", "TFvariable", "Z_score"]
            expression0 = "!1960_OR_BE! / !TOTAL_NUMB!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            ...
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Lone_Parent_Martial_2011_DA.shp"
        elif name == duplicated_name("Marital_DA", 1):
            ...
            keepFields = ["FID", "Shape", "CTUID", "CTNAME", "CMAUID", "CMANAME", "CMATYPE", "CMAPUID", "PRUID", "PRNAME", "OID_", "GEOGRAPHY", "UNEMPLOYED", "TOTAL_PO6", "Tvariable", "TFvariable", "Z_score"]
            expression0 = "!UNEMPLOYED! / !TOTAL_PO6!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            ...
            keepFields = ["FID", "Shape", "CTUID", "CTNAME", "CMAUID", "CMANAME", "CMATYPE", "CMAPUID", "PRUID", "PRNAME", "OID_", "GEOGRAPHY", "1960_OR_BE", "TOTAL_NUMB1", "Tvariable", "TFvariable", "Z_score"]
            expression0 = "!1960_OR_BE! / !TOTAL_NUMB1!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
