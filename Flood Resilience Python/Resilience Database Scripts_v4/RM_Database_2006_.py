...
def Tvariable_process(path):
    ...
    if name == duplicated_name("Income_DA", 0):
        ...
        Tvariable_identify(path)
        statsfield = arcpy.da.FeatureClassToNumPyArray(path, ["PREVALE14"])
        u = statsfield["PREVALE14"].mean()
        ...
        Tvariable_identify(path)
        statsfield = arcpy.da.FeatureClassToNumPyArray(path, ["2001_TO_20"])
        u = statsfield["2001_TO_20"].mean()
        ...
        Tvariable_identify(path)
        statsfield = arcpy.da.FeatureClassToNumPyArray(path, ["RENTED"])
        u = statsfield["RENTED"].mean()
        ...
        Tvariable_identify(path)
        statsfield0 = arcpy.da.FeatureClassToNumPyArray(path, ["PERIOD_OF_"])
        u0 = statsfield0["PERIOD_OF_"].mean()
        statsfield1 = arcpy.da.FeatureClassToNumPyArray(path, ["PERIOD_OF1"])
        u1 = statsfield1["PERIOD_OF1"].mean()
        cursor = arcpy.da.UpdateCursor(path, ["TOTAL_NU8", "PERIOD_OF_", "PERIOD_OF1", "Tvariable"])
        for row in cursor:
            if row[0] == 0:
                cursor.deleteRow()
            elif row[1] == -9999 or row[2] == -9999:
                row[1] = u0
                row[2] = u1
                row[3] = "N/A"
                cursor.updateRow(row)
        ...
        statsfield = arcpy.da.FeatureClassToNumPyArray(path, ["RENTED"])
        u = statsfield["RENTED"].mean()
        cursor = arcpy.da.UpdateCursor(path, ["TOTAL_NU4", "RENTED", "Tvariable"])
        for row in cursor:
            if row[0] == 0:
                cursor.deleteRow()
            elif row[1] == -9999:
