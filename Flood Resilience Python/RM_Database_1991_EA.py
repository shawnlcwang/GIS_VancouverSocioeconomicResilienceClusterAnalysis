...
def null_process(path):
    ...
    if name == duplicated_name("Dwelling_households_EA", 0):
        ...
        del row
        del cursor
    elif name == duplicated_name("Language_B_EA", 0):
        null_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["COL6", "Tvariable"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
        ...
        del row
        del cursor
    elif name == duplicated_name("Reli_ethnic_ori_citzn_immi_B_EA", 0):
        null_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["COL6", "Tvariable"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
        ...
        del row
        del cursor
    elif name == duplicated_name("Reli_ethnic_ori_citzn_immi_B_EA", 1):
        null_identify(path)
        cursor = arcpy.da.UpdateCursor(path, ["COL6", "Tvariable"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
...
def fcs_in_workspace(inputFolder):
            ...
            if fileName.endswith(suffix):
                ...
                for fc in fclist:
                    ...
                    if name == "Dwelling_households_EA":
                        ...
                        duplicate(name, fc, outputFolder, 1)
                    elif name == "Language_B_EA":
                        duplicate(name, fc, outputFolder, 1)
                    elif name == "Reli_ethnic_ori_citzn_immi_B_EA":
                        duplicate(name, fc, outputFolder, 2)
    ...
    for fc in fclist:
        ...
        if name == duplicated_name("Citizen_Immigration_Migration_CT", 0):
            ...
            arcpy.CalculateField_management(path, "normalize", "!COL63! / !COL4!", "PYTHON_9.3")
            ...
            arcpy.CalculateField_management(path, "normalize", "!COL17! / !COL6!", "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            ...
            arcpy.CalculateField_management(path, "normalize", "!COL12! / !COL6!", "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            ...
            arcpy.CalculateField_management(path, "normalize", "(!COL13! + !COL14!) / !COL6!", "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            ...
            arcpy.CalculateField_management(path, "normalize", "!COL51! / !COL47!", "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Living_Alone_Dwellings_1991_DA.shp"
            ...
            expression0 = "!COL20!"
            arcpy.CalculateField_management(path, "normalize", "!COL20!", "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            ...
            arcpy.CalculateField_management(path, "normalize", "!COL9! / !COL7!", "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            ...
            arcpy.CalculateField_management(path, "normalize", "!COL45! / !COL6!", "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            ...
            arcpy.CalculateField_management(path, "normalize", "!COL87! / !COL6!", "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            ...
            arcpy.CalculateField_management(path, "normalize", "!COL42! / !COL6!", "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
