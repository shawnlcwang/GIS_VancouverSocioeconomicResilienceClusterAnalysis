...
inputFolder = "F:\\MSc Thesis\\CCAR Database\\1991\\Resilience_1991_Input"
outputFolder = "F:\\MSc Thesis\\CCAR Database\\1991\\Resilience_1991_Output"
...
def data_process(path, keepFields, expression0, expression1, newName):
    ...
    fieldName0 = arcpy.ValidateFieldName("TFvariable")
    arcpy.AddField_management(path, fieldName0, "DOUBLE", "", "", 50)
    # Automate field calculator for normalized variable
    arcpy.CalculateField_management(path, fieldName0, expression0, "PYTHON_9.3")
    # Automate add normalized variable field
    fieldName1 = arcpy.ValidateFieldName("Z_score")
    arcpy.AddField_management(path, fieldName1, "DOUBLE", "", "", 50)
    # Automate field calculator for normalized variable
    arcpy.CalculateField_management(path, fieldName1, expression1, "PYTHON_9.3")
...
def null_value(path):
    ...
    if name == duplicated_name("Income_EA", 0):
        ...
        cursor = arcpy.da.UpdateCursor(path, ["COL54"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
                cursor.deleteRow()
    elif name == duplicated_name("Dwelling_households_B_EA", 0):
        cursor = arcpy.da.UpdateCursor(path, ["COL9"])
        for row in cursor:
            if row[0] == 0 or row[0] is None:
                cursor.deleteRow()
...
def fcs_in_workspace(inputFolder):
    ...
    for fc in fclist:
        ...
        if name == duplicated_name("Income_EA", 0):
            ...
            keepFields = ["FID", "Shape", "EA", "EA_STR", "OID_", "COL34", "COL27", "TFvariable", "Z_score"]
