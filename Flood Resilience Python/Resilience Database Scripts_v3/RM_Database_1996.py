...
# Define zero data process function using variable of path
def zero_process(path):
    cursor = arcpy.da.UpdateCursor(path, ["TFvariable"])
    for row in cursor:
        if row[0] == 0:
            cursor.deleteRow()
    del row
...
# Define missing data process function using variable of path
def null_process(path):
    statsfield = arcpy.da.FeatureClassToNumPyArray(path, ["TFvariable"])
    u = statsfield["TFvariable"].mean()
    cursor = arcpy.da.UpdateCursor(path, ["TFvariable"])
    for row in cursor:
        if row[0] is None:
            row[0] = u
...
def fcs_in_workspace(inputFolder):
    ...
    for fc in fclist:
        ...
        if name == duplicated_name("Income_EA", 0):
            ...
            keepFields = ["FID", "Shape", "EAUID", "GEOGRAPHY", "RENTED", "TOTAL_NU4", "TFvariable", "Z_score"]
            ...
            keepFields = ["FID", "Shape", "EAUID", "GEOGRAPHY", "MAJOR_REPA", "TOTAL_NU4", "TFvariable", "Z_score"]
            expression0 = "!MAJOR_REPA! / !TOTAL_NU4!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Dwellings_Major_Repair_Income_1996_DA.shp"
        elif name == duplicated_name("Income_EA", 4):
            keepFields = ["FID", "Shape", "EAUID", "GEOGRAPHY", "PERIOD_OF_", "PERIOD_OF1", "TOTAL_NU4", "TFvariable", "Z_score"]
            expression0 = "(!PERIOD_OF_! + !PERIOD_OF1!)/ !TOTAL_NU4!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Dwellings_1960Constructions_Income_1991_DA.shp"
        elif name == duplicated_name("Labour_Occupation_Education_EA", 0):
            keepFields = ["FID", "Shape", "EAUID", "GEOGRAPHY", "UNEMPLOYED", "IN_THE_LAB", "TFvariable", "Z_score"]
            expression0 = "!UNEMPLOYED! / !IN_THE_LAB!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Dwellings_Rental_Labour_1996_DA.shp"
        elif name == duplicated_name("Language_Immigration_Citizenships_EA", 0):
            keepFields = ["FID", "Shape", "EAUID", "GEOGRAPHY", "1991_1996_", "TOTAL_POPU", "TFvariable", "Z_score"]
            expression0 = "!1991_1996_! / !TOTAL_POPU!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Immigration_Language_1996_DA.shp"
        elif name == duplicated_name("Language_Immigration_Citizenships_EA", 1):
            keepFields = ["FID", "Shape", "EAUID", "GEOGRAPHY", "NEITHER_EN", "TOTAL_PO3", "TFvariable", "Z_score"]
            expression0 = "!NEITHER_EN! / !TOTAL_PO3!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Neither_Language_Language_1996_DA.shp"
        elif name == duplicated_name("Language_Immigration_Citizenships_EA", 2):
            keepFields = ["FID", "Shape", "EAUID", "GEOGRAPHY", "CANADIAN_C", "TOTAL_POPU", "TFvariable", "Z_score"]
            expression0 = "!CANADIAN_C! / !TOTAL_POPU!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Canadian_Citizen_Language_1996_DA.shp"
        elif name == duplicated_name("Marital_Families_Households_EA", 0):
            keepFields = ["FID", "Shape", "EAUID", "GEOGRAPHY", "TOTAL_LONE", "TOTAL_NUMB", "TFvariable", "Z_score"]
            expression0 = "!TOTAL_LONE! / !TOTAL_NUMB!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Lone_Parent_Marital_1996_DA.shp"
        elif name == duplicated_name("Marital_Families_Households_EA", 1):
            keepFields = ["FID", "Shape", "EAUID", "GEOGRAPHY", "LIVING_AL1", "TOTAL_NU3", "TFvariable", "Z_score"]
            expression0 = "!LIVING_AL1! / !TOTAL_NU3!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Living_Alone_Marital_1996_DA.shp"
        elif name == duplicated_name("Marital_Families_Households_EA", 2):
            keepFields = ["FID", "Shape", "EAUID", "GEOGRAPHY", "MOVABLE_DW", "TOTAL_NU4", "TFvariable", "Z_score"]
            ...
            keepFields = ["FID", "Shape", "GEOGRAPHY", "RENTED", "TOTAL_NU4", "TFvariable", "Z_score"]
            expression0 = "!RENTED! / !TOTAL_NU4!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            ...
            keepFields = ["FID", "Shape", "GEOGRAPHY", "MAJOR_REPA", "TOTAL_NU4", "TFvariable", "Z_score"]
            expression0 = "!MAJOR_REPA! / !TOTAL_NU4!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Dwellings_Major_Repair_Income_1996_CT.shp"
        elif name == duplicated_name("Income_CT", 6):
            keepFields = ["FID", "Shape", "GEOGRAPHY", "PERIOD_OF_", "PERIOD_OF1", "TOTAL_NU4", "TFvariable", "Z_score"]
            expression0 = "(!PERIOD_OF_! + !PERIOD_OF1!) / !TOTAL_NU4!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Dwellings_1960Constructions_Income_1996_CT.shp"
        elif name == duplicated_name("Labour_Occupation_Education_CT", 0):
            keepFields = ["FID", "Shape", "GEOGRAPHY", "UNEMPLOYED", "IN_THE_LAB", "TFvariable", "Z_score"]
            expression0 = "!UNEMPLOYED! / !IN_THE_LAB!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Labour_Unemployment_Labour_1996_CT.shp"
        elif name == duplicated_name("Language_Immigration_Citizenships_CT", 0):
            keepFields = ["FID", "Shape", "GEOGRAPHY", "1991_1996_", "TOTAL_POPU", "TFvariable", "Z_score"]
            expression0 = "!1991_1996_! / !TOTAL_POPU!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Immigration_Language_1996_CT.shp"
        elif name == duplicated_name("Language_Immigration_Citizenships_CT", 1):
            keepFields = ["FID", "Shape", "GEOGRAPHY", "NEITHER_EN", "TOTAL_PO3", "TFvariable", "Z_score"]
            expression0 = "!NEITHER_EN! / !TOTAL_PO3!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Neither_Language_Language_1996_CT.shp"
        elif name == duplicated_name("Language_Immigration_Citizenships_CT", 2):
            keepFields = ["FID", "Shape", "GEOGRAPHY", "CANADIAN_C", "TOTAL_POPU", "TFvariable", "Z_score"]
            expression0 = "!CANADIAN_C! / !TOTAL_POPU!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Canadian_Citizen_Language_1996_CT.shp"
        elif name == duplicated_name("Marital_Families_Households_CT", 0):
            keepFields = ["FID", "Shape", "GEOGRAPHY", "TOTAL_NUMB", "TOTAL_LONE", "TFvariable", "Z_score"]
            expression0 = "!TOTAL_LONE! / !TOTAL_NUMB!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Lone_Parent_Marital_1996_CT.shp"
        elif name == duplicated_name("Marital_Families_Households_CT", 1):
            keepFields = ["FID", "Shape", "GEOGRAPHY", "LIVING_AL1", "TOTAL_NU3", "TFvariable", "Z_score"]
            expression0 = "!LIVING_AL1! / !TOTAL_NU3!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
            newName = "Living_Alone_Marital_1996_CT.shp"
        elif name == duplicated_name("Marital_Families_Households_CT", 2):
            keepFields = ["FID", "Shape", "GEOGRAPHY", "MOVABLE_DW", "TOTAL_NU4", "TFvariable", "Z_score"]
            expression0 = "!MOVABLE_DW! / !TOTAL_NU4!"
            arcpy.CalculateField_management(path, "normalize", expression0, "PYTHON_9.3")
            expression1 = variable_calculate(path, "normalize")
